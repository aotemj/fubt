    ...
    <div class="security">
      ...
      <ul class="subMenu">
        ...
        <li>
          ...
          <el-dialog title="绑定手机" :visible.sync="phoneFormVisible" width="40%" center>
            <el-form :model="form" class="modifyPassword">
              ...
              <el-form-item label="原手机号码" class="modify" style="text-algin:left;">
        ...
        <li>
          ...
          <span class="floatRight" @click="RealFormVisible = true" v-if="userInfo.frealname == null">修改</span>
          <span v-else></span>
          <el-dialog title="实名认证：" :visible.sync="RealFormVisible" width="30%" center>
            <el-form :model="form" class="modifyPassword">
              ...
              <el-form-item label="证件类型" class="modify">
                <select class="modifyInput" v-model="identitytype">
...
export default {
  ...
  methods: {
    ...
    loginPassword() {
      ...
      ajax(regUrl, "post", fda, res => {
        if (res.data.code !== 200) {
          ...
          this.$store.commit("changeDialogInfo", {
            dataInfo: "修改成功",
            skipurl: "login"
          });
    ...
    phoneVisible() {
      ...
      bindfd.append("areaCode", "中国大陆(China)"); //地区代码
      bindfd.append("phone", this.phoneNum); //手机号
      bindfd.append("imgcode", this.imgCode); //图片验证码
      bindfd.append("newcode", this.bsendCode); //短信验证码
    ...
    bindsendCode() {
      ...
      bindsendfd.append("vcode", this.imgCode);
      bindsendfd.append("uid", 0);
      bindsendfd.append("imageRedisKey", this.imageRedisKey);
      /*
      type: 102
      msgtype: 1
      areaCode: +86
    ...
    Realnamebox() {
      ...
      Rfd.append("realname", this.realname); // 用户名
      Rfd.append("identitytype", "0"); //验证类型
      Rfd.append("identityno", this.identityno); // 身份证号
      Rfd.append("address", "中国大陆(China)"); //地区
